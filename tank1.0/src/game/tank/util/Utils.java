...
public class Utils
{
     ...
     * @param scene
     */
    public static void moveTank(Tank tank, int step, int direction, int speed,
            Scene scene)
    {
            ...
            List<Tank> tankList, int speed, Scene scene)
    {
        ...
        for (int i = 0; i < step; i++)
        {
            try
            {
                virtualTank.setLastPosition(new Point(virtualTank.getHorizon(),
                        virtualTank.getVertical()));
                changeLocation(virtualTank, direction);
                if (checkCollide(virtualTank, scene.getMapElements())
                        || checkCollide(virtualTank, tankList))
                {
                    virtualTank.undo();
                    return;
                }
                tank.setLastPosition(new Point(tank.getHorizon(), tank
                        .getVertical()));
    ...
    public static Scene buildScene(int round)
    {
        ...
        Tank main = TankFactory.buildTank(TankType.MAIN_TANK, 160, 464, 1,
                false);
        sce.addTank(main);
        return sce;
    }
